...
impl CmaOptimizer {
    ...
    pub fn new(device: Arc<CudaDevice>, dimensions: usize, seed: u64) -> Result<Self> {
        ...
        // Initialize sigma
        device.htod_copy_into(vec![params.sigma], &mut d_sigma)?;
        device.htod_copy_into(vec![f32::INFINITY], &mut d_best_fitness)?;
        device.htod_copy_into(vec![1.0f32], &mut d_condition)?;
        ...
        device.htod_copy_into(identity.clone(), &mut d_covariance)?;
    ...
    pub fn step(&mut self, fitness_fn: impl Fn(&[f32]) -> f32) -> Result<()> {
        ...
        self.device
            .htod_copy_into(fitness.clone(), &mut self.d_fitness)?;
