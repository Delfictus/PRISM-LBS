...
impl PrismPipeline {
    ...
    pub fn new(config: PrismConfig) -> Result<Self> {
        ...
        let fusion_kernel = Arc::new(context.get_func("prism_fusion", "_Z23fuse_coherence_matricesPKfS0_S0_S0_Pfiffff")
            .map_err(|e| anyhow!("Failed to load fuse_coherence_matrices: {:?}", e))?);

        let init_kernel = Arc::new(context.get_func("prism_fusion", "_Z22init_uniform_coherencePfif")
    ...
    fn fuse_coherence_gpu(
        ...
        let device = &*self.context;  // Deref Arc<CudaDevice>

        // Upload coherence matrices to GPU
        let topo_gpu: CudaSlice<f32> = device.htod_sync_copy(topological)?;
        let causal_gpu: CudaSlice<f32> = device.htod_sync_copy(causal)?;
        let neuro_gpu: CudaSlice<f32> = device.htod_sync_copy(neuromorphic)?;
        let gnn_gpu: CudaSlice<f32> = device.htod_sync_copy(gnn)?;

        // Allocate output on GPU
        let mut enhanced_gpu: CudaSlice<f32> = device.alloc_zeros(n * n)?;
        ...
        unsafe {
            self.fusion_kernel.clone().launch(config, (
                &topo_gpu,
                &causal_gpu,
                &neuro_gpu,
                &gnn_gpu,
        ...
        device.synchronize()?;
        let enhanced: Vec<f32> = device.dtoh_sync_copy(&enhanced_gpu)?;
