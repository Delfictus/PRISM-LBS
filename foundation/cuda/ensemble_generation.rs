...
impl GpuEnsembleGenerator {
    ...
    pub fn new() -> Result<Self> {
        ...
        let kernel_names = vec!["generate_thermodynamic_ordering"];
        context.load_ptx(ptx, "ensemble_gen", &kernel_names)
            .map_err(|e| anyhow!("Failed to load PTX module: {:?}", e))?;

        let generate_kernel = Arc::new(context.get_func("ensemble_gen", "generate_thermodynamic_ordering")
    ...
    pub fn generate(
        ...
        let degrees_gpu: CudaSlice<f32> = device.htod_sync_copy(&degrees_f32)?;

        // Allocate output buffers on GPU
        let mut orderings_gpu: CudaSlice<i32> = device.alloc_zeros(ensemble_size * n)?;
        let mut energies_gpu: CudaSlice<f32> = device.alloc_zeros(ensemble_size)?;
        ...
        let temps_gpu: CudaSlice<f32> = device.htod_sync_copy(&temperatures)?;
        ...
        let seeds_gpu: CudaSlice<u64> = device.htod_sync_copy(&seeds)?;
        ...
        unsafe {
            self.generate_kernel.clone().launch(config, (
                &degrees_gpu,
                &temps_gpu,
                &seeds_gpu,
                &mut orderings_gpu,
        ...
        // Download results
        let orderings_flat: Vec<i32> = device.dtoh_sync_copy(&orderings_gpu)?;
        let energies: Vec<f32> = device.dtoh_sync_copy(&energies_gpu)?;
