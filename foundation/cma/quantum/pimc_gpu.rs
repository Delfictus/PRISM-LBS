...
impl GpuPathIntegralMonteCarlo {
    ...
    pub fn new(n_beads: usize, beta: f64) -> Result<Self> {
        ...
        // Load PTX module with kernel names
        let kernel_names = vec![
            "init_rand_states_kernel",
            "update_beads_kernel",
        ];

        device.load_ptx(ptx.into(), "pimc_module", &kernel_names)
    ...
    pub fn quantum_anneal_gpu(
        ...
        let init_rand = self.device.get_func("pimc_module", "init_rand_states_kernel")?;
        ...
        unsafe {
            init_rand.clone().launch(init_config, (
                &rand_states_gpu,
                &seed,
                &path_size_i32,
            ))?;
        ...
        let update_func = self.device.get_func("pimc_module", "update_beads_kernel")?;
        ...
        for step in 0..n_steps {
            ...
            unsafe {
                update_func.clone().launch(config, (
                    &path_gpu,
                    &h_gpu,
                    &manifold_gpu,
                    &manifold_edges,
