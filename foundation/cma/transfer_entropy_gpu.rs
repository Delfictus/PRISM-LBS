...
impl GpuKSGEstimator {
    ...
    pub fn new(k: usize, embed_dim: usize, delay: usize) -> Result<Self> {
        ...
        device.load_ptx(ptx.into(), "ksg_module", &kernel_names)
            .context("Failed to load KSG CUDA module")?;

        let mut kernels = HashMap::new();
        for &name in &kernel_names {
            let func = device.get_func("ksg_module", name)?;
    ...
    pub fn compute_te_gpu(&self, source: &TimeSeries, target: &TimeSeries) -> Result<TransferEntropyResult> {
        ...
        let mut distances_gpu = self.device.alloc_zeros::<f32>(n_points * n_points)?;
        let mut epsilon_values_gpu = self.device.alloc_zeros::<f32>(n_points)?;

        // Step 1: Compute pairwise distances
        let threads = 256;
        let blocks = (n_points + threads - 1) / threads;

        let compute_dist_func = &self.kernels["compute_distances_kernel"];
        ...
        unsafe {
            compute_dist_func.clone().launch(config, (
                &y_current_gpu,
                &y_past_gpu,
                &x_past_gpu,
                &mut distances_gpu,
        ...
        let find_kth_func = &self.kernels["find_kth_distance_kernel"];
        let k_i32 = self.k as i32;

        unsafe {
            find_kth_func.clone().launch(config, (
                &distances_gpu,
                &mut epsilon_values_gpu,
                n_points_i32,
                k_i32,
            ))?;
        }
        ...
        let mut counts_y_gpu = self.device.alloc_zeros::<i32>(n_points)?;
        let mut counts_xz_gpu = self.device.alloc_zeros::<i32>(n_points)?;
        let mut counts_z_gpu = self.device.alloc_zeros::<i32>(n_points)?;

        let count_y_func = &self.kernels["count_neighbors_y_kernel"];
        let count_xz_func = &self.kernels["count_neighbors_xz_kernel"];
        let count_z_func = &self.kernels["count_neighbors_z_kernel"];

        unsafe {
            count_y_func.clone().launch(config, (
                &y_current_gpu,
                &y_past_gpu,
                &epsilon_values_gpu,
                &mut counts_y_gpu,
        ...
        unsafe {
            count_xz_func.clone().launch(config, (
                &x_past_gpu,
                &y_past_gpu,
                &epsilon_values_gpu,
                &mut counts_xz_gpu,
        ...
        unsafe {
            count_z_func.clone().launch(config, (
                &y_past_gpu,
                &epsilon_values_gpu,
                &mut counts_z_gpu,
                n_points_i32,
        ...
        let mut te_contributions_gpu = self.device.alloc_zeros::<f32>(n_points)?;

        let compute_te_func = &self.kernels["compute_te_kernel"];

        unsafe {
            compute_te_func.clone().launch(config, (
                &counts_y_gpu,
                &counts_xz_gpu,
                &counts_z_gpu,
                &mut te_contributions_gpu,
                n_points_i32,
                k_i32,
            ))?;
        }
        ...
        let mut te_result_gpu = self.device.alloc_zeros::<f32>(1)?;

        let reduce_func = &self.kernels["reduce_sum_kernel"];
        ...
        unsafe {
            reduce_func.clone().launch(reduce_config, (
