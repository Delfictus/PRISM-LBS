...
impl NeuromorphicAdapter {
    pub fn new_gpu(
        ...
        {
            anyhow::bail!("GpuReservoirComputer unavailable without CUDA feature")
        }
    }
}
...
impl ThermodynamicAdapter {
    pub fn new_gpu(context: Arc<CudaDevice>, n_oscillators: usize) -> Result<Self> {
...
impl QuantumAdapter {
    pub fn new_gpu(_context: Arc<CudaDevice>, num_qubits: usize) -> Result<Self> {
        #[cfg(feature = "quantum_mlir_support")]
        {
        ...
        #[cfg(not(feature = "quantum_mlir_support"))]
        {
            let state_dim = 1 << num_qubits;
            Ok(Self {
                phases: Array1::zeros(state_dim),
