...
use crate::foundation::statistical_mechanics::ThermodynamicState;
...
pub struct UnifiedPlatform {
    /// Shared CUDA device (GPU resources)
...
impl UnifiedPlatform {
    ...
    pub fn new_with_device(n_dimensions: usize, device_id: usize) -> Result<Self> {
        ...
        // Step 1: Create shared CUDA device for specified device
        let cuda_device = Arc::new(CudaDevice::new(device_id)
            .map_err(|e| anyhow!("Failed to create CUDA device on device {}: {}", device_id, e))?);
        println!("[Platform] ✓ CUDA device created (device {})", device_id);
        ...
        let neuromorphic = Box::new(
            NeuromorphicAdapter::new_gpu(cuda_device.clone(), n_dimensions, 1000)?
        ...
        let information_flow = Box::new(
            InformationFlowAdapter::new_gpu(cuda_device.clone(), 10, 1, 1)?
        ...
        let active_inference = Box::new(
            ActiveInferenceAdapter::new_gpu(cuda_device.clone(), n_dimensions)?
