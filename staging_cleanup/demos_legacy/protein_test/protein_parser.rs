...
/// Convenience function to parse PDB file with default contact distance
pub fn parse_pdb_file(path: &Path) -> Result<ProteinContactGraph> {
    const DEFAULT_CONTACT_DISTANCE: f64 = 8.0; // Angstroms
    ProteinContactGraph::from_pdb_file(path, DEFAULT_CONTACT_DISTANCE)
}
